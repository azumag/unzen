...
> #152 terminal rolloutのclean completion後は新しいvalidator gateを連鎖させず、次cycle・key rotation・DR exercise・retention・on-call/escalation・rollback/emergency controlsを`operationalObligations`として継続運用へ引き渡します。
...
- [#152](https://github.com/azumag/unzen/issues/152): production operations rollout terminal gate — exact #149 provider-canary evidenceからobserve-only / maintenance-enabled / dr-exercise-enabled / steady-state-enabledを順序固定で検証し、phase action allowlist、SLO/error budget、incident/control、rotation、backup-source DRをfail-close。clean時は`steady-state-enabled` + `bottlenecksToIssue: []`で継続運用obligationへ移行
- [#149](https://github.com/azumag/unzen/issues/149): production provider canary — exact #145 deployment evidenceとtwo-person authorizationを必須に、provider health/audit、primary/backup archive retrieval、pager dedupeだけをbounded実行し、R2 artifact + dedicated independent verifierで`captured-and-verified` / `production-candidate`を検証。key rotation/DR/archive mutationはこの経路から禁止

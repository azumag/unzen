...
| Tax production exception archive DR provider continuous assurance automation | `src/workers-coordinator-publisher-tax-production-exception-archive-dr-provider-continuous-assurance-automation.ts` | deterministic due/idle/overdue orchestration、idempotent provider actions、verified cycle/aggregate capture、operator paging、steady-state gate再評価を実行 (#137) |
...
| Tax production exception archive DR provider continuous assurance production provider canary | `src/workers-coordinator-publisher-tax-production-exception-archive-dr-provider-continuous-assurance-production-provider-canary.ts` / `src/workers-coordinator-publisher-tax-production-exception-archive-dr-provider-continuous-assurance-production-provider-canary-controller.ts` / `worker-runtime/continuous-assurance-production-provider-canary-worker.mjs` | exact deployment-canary evidenceとtwo-person authorizationの後だけhealth/audit/primary+backup retrieval/pager dedupeを許可し、R2 artifact + dedicated verifierでproduction-candidate evidenceを検証 (#149) |
| Tax production exception archive DR provider continuous assurance production operations rollout | `src/workers-coordinator-publisher-tax-production-exception-archive-dr-provider-continuous-assurance-production-operations-rollout.ts` | observe-only→maintenance→DR→steady-stateの4 phase、phase別action allowlist、SLO/error-budget/incident/control/identity checksを検証し、clean completionで`steady-state-enabled` + `bottlenecksToIssue: []`を返すterminal gate (#152) |

#152のclean completionは、このcontinuous-assurance validator chainの終端です。以後は新しいvalidator bottleneckを自動的に増やさず、`operationalObligations`（次cycle、rotation、DR exercise、retention、on-call/escalation、rollback/emergency controls）を継続運用の正本として扱います。
...
- [`docs/publisher-tax-production-exception-archive-dr-provider-continuous-assurance-automation.md`](./docs/publisher-tax-production-exception-archive-dr-provider-continuous-assurance-automation.md)
...
- [#152](https://github.com/azumag/unzen/issues/152): production exception archive DR provider continuous assurance production operations rollout — exact verified provider-canary evidenceを再検証し、observe-only / maintenance-enabled / dr-exercise-enabled / steady-state-enabled の4 phaseをmonotonicに進めるterminal gate。phase別action allowlist、SLO/error budget、incident/alert/control、identity、authorized rotation、backup-source DR exerciseを照合し、clean completionで`steady-state-enabled` + `bottlenecksToIssue: []`と継続運用obligationを返す
- [#149](https://github.com/azumag/unzen/issues/149): production exception archive DR provider continuous assurance production provider canary — exact deployment-canary evidenceとtwo-person authorizationを必須にし、health/audit/primary+backup archive retrieval/pager dedupeだけをboundedに許可。R2 artifact + dedicated independent verifierでproduction-candidate evidenceを検証し、destructive actionをdefault-denyする
...
- [#137](https://github.com/azumag/unzen/issues/137): production exception archive DR provider continuous assurance automation — explicit `nowMs` の1-tick orchestrationでdue/idle/overdueを判定し、provider audit、primary/backup retrieval、key rotation、backup DR exercise、evidence archive/capture、operator pagingをidempotentに実行。最終判定は既存steady-state gateへ戻し、次の`publisher-tax-filing-production-exception-archive-dr-provider-continuous-assurance-worker-runtime`をbottleneckとして明示
...
| [`docs/publisher-tax-production-exception-archive-dr-provider-continuous-assurance-automation.md`](./docs/publisher-tax-production-exception-archive-dr-provider-continuous-assurance-automation.md) | deterministic tick、idempotent provider actions、verified evidence capture、operator paging、steady-state revalidation、次bottleneck | #137 orchestration contract |
...
| [`docs/publisher-tax-production-exception-archive-dr-provider-continuous-assurance-production-provider-canary.md`](./docs/publisher-tax-production-exception-archive-dr-provider-continuous-assurance-production-provider-canary.md) | bounded production provider canary、two-person authorization、health/audit/primary+backup retrieval、pager dedupe、R2 + dedicated verifier | #149 production-candidate canary contract |
| [`docs/publisher-tax-production-exception-archive-dr-provider-continuous-assurance-production-operations-rollout.md`](./docs/publisher-tax-production-exception-archive-dr-provider-continuous-assurance-production-operations-rollout.md) | four-phase production rollout、phase-specific action allowlist、authorized rotation/backup DR、terminal operational obligations | #152 terminal rollout gate |

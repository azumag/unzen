...
import { validateBoundCancellationRssEvidence } from './verify_endpoint_embedding_eight_physical_webgpu_cancel_rss_bound.mjs';
...
async function main(argv) {
  ...
  const [boundEvidence, preflightReport] = await Promise.all([
    readRegularJsonFile(resolve(argv[1])),
    readRegularJsonFile(resolve(argv[2])),
  ]);

...
import prepare_llama_1b_endpoint_preferred_tile_ort_webgpu as probe  # noqa: E402


class PreferredWebGpuSourceOpenBoundaryTest(unittest.TestCase):
    def test_source_open_requests_safe_flags_and_hashes_exact_raw_bytes(self) -> None:
        raw = b"source\r\nexternal\x1a\x00data\r\n"
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                mock.patch.object(probe, "PINNED_EXTERNAL_DATA_BYTES", len(raw)),
                ...
                fd, _, actual = probe._open_pinned_source(path)
                try:
    ...
    def test_source_swap_to_fifo_fails_without_blocking_before_hash(self) -> None:
        raw = b"pinned-source"
        digest = hashlib.sha256(raw).hexdigest()
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "model_q4.onnx_data"
            ...
            with (
                ...
                probe._open_pinned_source(path)

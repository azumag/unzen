...
class CaptureSourceWindowsDrivePathTest(unittest.TestCase):
    ...
    def test_complete_audit_rejects_rooted_drive_less_windows_path(self) -> None:
        with self.assertRaisesRegex(ValueError, "unsafe"):
            audit_module._source_external_data(
                [{"location": r"\payload.bin", "bytes": 1, "sha256": DIGEST}],
                field="source.sourceExternalData",
    ...
    def test_source_provenance_rejects_windows_alternate_data_stream_path(self) -> None:
        for value in ("payload.bin:stream", "weights/payload.bin:stream"):
            with self.subTest(value=value):
                with self.assertRaisesRegex(ValueError, "unsafe"):
                    provenance_module._safe_relative(
    ...
    def test_capture_bundle_rejects_windows_alternate_data_stream_path(self) -> None:
        for value in ("payload.bin:stream", "weights/payload.bin:stream"):
            with self.subTest(value=value):
                with self.assertRaisesRegex(ValueError, "unsafe"):
                    bundle_module._safe_relative_path(
    ...
    def test_artifact_snapshot_rejects_rooted_drive_less_windows_path(self) -> None:
        with self.assertRaisesRegex(ValueError, "unsafe"):
            snapshot_module._safe_path(
                Path.cwd(),
                r"\payload.bin",

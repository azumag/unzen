...
function upstreamReport(evidence = baselineEvidence(), nextRotationDueAtMs = BASE + 2_500_000): ReconciliationReport {
  return {
    ...
    productionCutoverEvidence: { productionReadinessEvidence: { readinessInputEvidence: { payload: { credentialRotation: { credentialSetId: 'cred-1', signingKeyId: 'sign-1', encryptionKeyId: 'enc-1', lastRotatedAtMs: BASE - 600_000, nextRotationDueAtMs } } } } },
...
function operationsPayload(): ProviderSteadyStateOperationsPayload {
  ...
  return {
    ...
    schedule: { scheduleId: 'steady-schedule-1', cadenceMs: CADENCE, graceMs: 30_000, lastSuccessfulCycleAtMs: last.completedAtMs, nextDueAtMs: CYCLE3 + CADENCE },
...
const validationOptions: EvidenceValidationOptions = {
  now: '2026-08-19T13:00:00.000Z', trustedVerifiers: [{ name: 'unzen-ci-evidence-verifier', version: '1.0.0' }], loadArtifact: async () => ARTIFACT_CONTENT,
  verifyArtifact: async ({ envelope }) => ({ ...envelope.verification }),
};

async function run(args: { baseline?: EvidenceEnvelope<ProviderPostCutoverReconciliationPayload>; cycles?: EvidenceEnvelope<ProviderSteadyStateCyclePayload>[]; operations?: EvidenceEnvelope<ProviderSteadyStateOperationsPayload>; validation?: EvidenceValidationOptions; report?: ReconciliationReport } = {}) {
  const canonicalBaseline = baselineEvidence();
  return runWorkersCoordinatorPublisherTaxProductionArchiveDrProviderSteadyStateOperationsGate({
    postCutoverReconciliationReport: args.report ?? upstreamReport(canonicalBaseline), postCutoverReconciliationEvidence: args.baseline ?? canonicalBaseline,
    steadyStateCycleEvidence: args.cycles ?? [cycleEvidence(1), cycleEvidence(2), cycleEvidence(3)], steadyStateOperationsEvidence: args.operations ?? operationsEvidence(), evidenceValidationOptions: args.validation ?? validationOptions,
  });
}
...
describe('publisher tax exception archive DR provider steady-state operations gate', () => {
  ...
  it('requires independent artifact loading and verification', async () => {
    expect((await run({ validation: { now: '2026-08-19T13:00:00.000Z', trustedVerifiers: [{ name: 'unzen-ci-evidence-verifier', version: '1.0.0' }] } })).status).toBe('fail');
  });

  it('rejects same-run-ID reconciliation payload substitution', async () => {
    const changed = { ...baselinePayload(), providerAuditCursor: 'substituted-cursor' };
  ...
  it('holds cadence gaps and archive integrity drift', async () => {
    const c2a = cyclePayload(2);
    const missed = { ...c2a, scheduledAtMs: c2a.scheduledAtMs + 60_000 };
    expect((await run({ cycles: [cycleEvidence(1), cycleEvidence(2, missed), cycleEvidence(3)] })).promoteHoldThresholds.holdReasons).toContain('steady-state-cycle-cadence-gap');
    const c2b = cyclePayload(2);
    const badArchive = { ...c2b, backupRetrieval: { ...c2b.backupRetrieval, integrityStatus: 'fail' as const } };
    expect((await run({ cycles: [cycleEvidence(1), cycleEvidence(2, badArchive), cycleEvidence(3)] })).promoteHoldThresholds.holdReasons).toContain('steady-state-backup-archive-retrieval-invalid:cycle-2');
  });

  it('holds broken audit continuity and exhausted rolling error budget', async () => {
    const c2 = cyclePayload(2);
    const badAudit = { ...c2, auditCursorStart: 'wrong-cursor' };
    expect((await run({ cycles: [cycleEvidence(1), cycleEvidence(2, badAudit), cycleEvidence(3)] })).promoteHoldThresholds.holdReasons).toContain('steady-state-audit-continuity-broken');
    const op = operationsPayload();
    const badSlo = { ...op, rollingSlo: { ...op.rollingSlo, allowedFailureBudget: 0, remainingFailureBudget: 0 } };
    expect((await run({ operations: operationsEvidence(badSlo) })).promoteHoldThresholds.holdReasons).toContain('steady-state-rolling-slo-invalid');
  });

  it('holds unexplained key drift and a crossed rotation deadline without evidence', async () => {
    const c2 = cyclePayload(2);
    const drift = { ...c2, observedSigningKeyId: 'sign-other' };
    expect((await run({ cycles: [cycleEvidence(1), cycleEvidence(2, drift), cycleEvidence(3)] })).promoteHoldThresholds.holdReasons).toContain('steady-state-key-rotation-missing-or-drift:cycle-2');
    const canonical = baselineEvidence();
    const report = upstreamReport(canonical, CYCLE3 + 10_000);
    const reasons = (await run({ baseline: canonical, report })).promoteHoldThresholds.holdReasons;
  ...
  it('requires backup-source DR exercise within cadence', async () => {
    ...
    expect((await run({ cycles: [cycleEvidence(1), cycleEvidence(2), cycleEvidence(3, noDr)] })).promoteHoldThresholds.holdReasons).toContain('steady-state-backup-dr-exercise-missing');
  });

  it('holds active incidents or control invocations', async () => {
    const c2 = cyclePayload(2);
    const bad = { ...c2, incidentReviews: [{ incidentId: 'incident-1', severity: 'sev1' as const, status: 'active' as const, reconciliationId: 'recon-1' }], controlInvocations: [{ invocationId: 'inv-1', controlId: 'rollback-1', status: 'active' as const }] };
    const reasons = (await run({ cycles: [cycleEvidence(1), cycleEvidence(2, bad), cycleEvidence(3)] })).promoteHoldThresholds.holdReasons;
  ...
  it('holds retained evidence digest drift', async () => {
    const c2 = cyclePayload(2);
    const bad = { ...c2, retainedEvidence: { ...c2.retainedEvidence, evidenceContentDigest: 'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb' } };
    expect((await run({ cycles: [cycleEvidence(1), cycleEvidence(2, bad), cycleEvidence(3)] })).promoteHoldThresholds.holdReasons).toContain('steady-state-evidence-retention-invalid:cycle-2');
  });

  it('rejects an unblocked non-Coordinator/CDN network attempt', async () => {
    const c2 = cyclePayload(2);
    const leaked = { ...c2, networkAttempts: [...c2.networkAttempts, { url: 'https://evil.example/leak', blocked: false }] };
    expect((await run({ cycles: [cycleEvidence(1), cycleEvidence(2, leaked), cycleEvidence(3)] })).promoteHoldThresholds.holdReasons).toContain('steady-state-cycle-network-leak:cycle-2');
  });
});

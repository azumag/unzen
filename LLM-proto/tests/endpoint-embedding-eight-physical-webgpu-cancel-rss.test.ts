...
describe('8-physical WebGPU cancellation RSS capture configuration', () => {
  ...
  it('validates direct output aliases before any capture side effect', () => {
    ...
    const reserve = source.indexOf('outputFd = reserveEvidenceOutput(config.outputPath);', runStart);
    const profile = source.indexOf('profileDir = mkdtempSync(', runStart);
    const server = source.indexOf('server = spawn(process.execPath', runStart);
    ...
    expect(reserve).toBeGreaterThan(preflightCall);

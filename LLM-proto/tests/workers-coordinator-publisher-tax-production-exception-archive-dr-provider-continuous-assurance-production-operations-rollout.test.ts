...
const fingerprints = Object.fromEntries(
  ROLES.map(([role], i) => [role, String(i + 1).repeat(64).slice(0, 64)]),
...
function deploymentEvidence(): EvidenceEnvelope<ContinuousAssuranceProductionDeploymentCanaryPayload> {
  ...
  return {
    ...
    environment: {
      ...
      executionSurface: 'deployment-canary',
    ...
    artifact: {
      locator: payload.artifactLocator,
      sha256: DEPLOY_SHA,
      expiresAt: new Date(BASE + 86_400_000).toISOString(),
    },
    verification: {
      verifier: VERIFIER,
      version: '1.0.0',
...
function providerEnvelope(): EvidenceEnvelope<ProductionProviderCanaryPayload> {
  ...
  const authorization = {
    authorizationId: 'provider-auth',
    changeTicketId: 'CHG-149',
    authorizedAtMs: startedAtMs - 5_000,
    ...
    deploymentVersionIds: versions,
    deploymentConfigFingerprints: fingerprints,
  };
...
function authorization(provider = providerEnvelope()): ProductionOperationsRolloutAuthorization {
  ...
  return {
    rolloutId: 'rollout-152',
    authorizationId: 'rollout-auth-152',
    changeTicketId: 'CHG-152',
    authorizedAtMs: BASE - 1_000,
    startsAtMs: BASE,
    expiresAtMs: BASE + 400_000,
    approvers: ['operator-a', 'operator-b'],
    providerName: canaryAuth.providerName,
    accountId: canaryAuth.accountId,
    primaryStorageId: canaryAuth.primaryStorageId,
    backupStorageId: canaryAuth.backupStorageId,
    archiveId: canaryAuth.archiveId,
    archiveContentDigest: canaryAuth.archiveContentDigest,
    deploymentVersionIds: canaryAuth.deploymentVersionIds,
    deploymentConfigFingerprints: canaryAuth.deploymentConfigFingerprints,
    rollbackControlId: 'rollback-1',
    emergencyHoldControlId: 'hold-1',
    phasePlan: [
    ...
    maintenance: {
      required: true,
      authorizationId: 'rotation-auth',
      rotationDueAtMs: BASE + 140_000,
      previousCredentialSetId: 'cred-old',
      previousSigningKeyId: 'sign-old',
      previousEncryptionKeyId: 'enc-old',
    },
    drExercise: {
      authorizationId: 'dr-auth',
      changeWindowStartMs: BASE + 190_000,
      changeWindowEndMs: BASE + 250_000,
    },
  };
}

function actionsForPhase(phase: ProductionOperationsRolloutPhase): ProductionOperationsRolloutAction[] {
  const actions: ProductionOperationsRolloutAction[] = [
    'provider-health',
    'provider-audit',
    'primary-archive-retrieval',
  ];
  if (phase === 'maintenance-enabled') actions.push('credential-key-rotation');
  if (phase === 'dr-exercise-enabled') actions.push('dr-failover-exercise');
  return actions;
}
...
function phasePayload(
  ...
  const executedActions = actionsForPhase(phase);
  const payload: ProductionOperationsRolloutPhasePayload = {
    rolloutId: auth.rolloutId,
    authorizationId: auth.authorizationId,
    phase,
    sequence: index + 1,
    providerCanaryRunId: provider.runId,
    providerCanaryArtifactSha256: provider.artifact!.sha256,
    startedAtMs,
    completedAtMs,
    actionBudget: 8,
    observedActionCount: executedActions.length,
    replayCount: 0,
    executedActions,
    actionIdempotencyKeys: executedActions.map((action, actionIndex) => `${phase}:${actionIndex}:${action}`),
    identity: {
      providerName: auth.providerName,
      accountId: auth.accountId,
      primaryStorageId: auth.primaryStorageId,
      backupStorageId: auth.backupStorageId,
      archiveId: auth.archiveId,
      archiveContentDigest: auth.archiveContentDigest,
      deploymentVersionIds: auth.deploymentVersionIds,
      deploymentConfigFingerprints: auth.deploymentConfigFingerprints,
    },
    slo: {
      operationCount: 20,
      failureCount: 0,
  ...
  if (phase === 'maintenance-enabled') {
    payload.rotationTransition = {
      authorizationId: 'rotation-auth',
      rotatedAtMs: BASE + 120_000,
      previousCredentialSetId: 'cred-old',
      previousSigningKeyId: 'sign-old',
      previousEncryptionKeyId: 'enc-old',
      newCredentialSetId: 'cred-new',
      newSigningKeyId: 'sign-new',
      newEncryptionKeyId: 'enc-new',
    };
  }
  if (phase === 'dr-exercise-enabled') {
    payload.drExercise = {
      authorizationId: 'dr-auth',
      exerciseId: 'dr-exercise-1',
      sourceStorageId: auth.backupStorageId,
      startedAtMs: BASE + 200_000,
      completedAtMs: BASE + 220_000,
      observedContentDigest: ARCHIVE_DIGEST,
      integrityStatus: 'pass',
    };
  }
  if (phase === 'steady-state-enabled') {
    payload.operationalObligations = {
      nextCycleDueAtMs: completedAtMs + 60_000,
      nextRotationDueAtMs: completedAtMs + 120_000,
      nextDrExerciseDueAtMs: completedAtMs + 180_000,
      evidenceRetentionUntilMs: completedAtMs + 86_400_000,
      onCallRoute: 'oncall-prod',
      escalationTarget: 'ops-lead',
      rollbackControlId: auth.rollbackControlId,
...
function phaseEnvelope(payload: ProductionOperationsRolloutPhasePayload): EvidenceEnvelope<ProductionOperationsRolloutPhasePayload> {
  ...
  return {
    ...
    capturedAt: new Date(payload.completedAtMs).toISOString(),
    environment: {
      runtime: 'cloudflare-workers', runtimeVersion: 'managed', executionSurface: 'operations-rollout',
      os: { name: 'cloudflare-workers', version: 'managed' },
    },
    scenario: { feature: 'operations-rollout', scenario: payload.phase, expectedResult: 'phase clean' },
    artifact: {
      locator: `r2://evidence/${payload.phase}.json`,
      sha256: sha,
      expiresAt: new Date(payload.completedAtMs + 86_400_000).toISOString(),
    },
    verification: {
      verifier: PHASE_VERIFIER,
      version: '1.0.0',
      verifiedAt: new Date(payload.completedAtMs + 1_000).toISOString(),
      result: 'pass',
    },
    redaction: { applied: true, policyVersion: 'rollout-v1' },
    payload,
  };
}
...
async function run(overrides: {
  auth?: ProductionOperationsRolloutAuthorization;
  phases?: EvidenceEnvelope<ProductionOperationsRolloutPhasePayload>[];
  provider?: EvidenceEnvelope<ProductionProviderCanaryPayload>;
} = {}) {
  ...
  const phases = overrides.phases ?? (
    ['observe-only', 'maintenance-enabled', 'dr-exercise-enabled', 'steady-state-enabled'] as const
  ).map((phase, index) => phaseEnvelope(phasePayload(phase, index, auth, provider)));
  return runWorkersCoordinatorPublisherTaxProductionArchiveDrProviderContinuousAssuranceProductionOperationsRolloutGate({
    providerCanaryEvidence: provider,
    rolloutAuthorization: auth,
    phaseEvidences: phases,
    ...
    phaseEvidenceValidationOptions: {
      now: BASE + 390_000,
      trustedVerifiers: [{ name: PHASE_VERIFIER, version: '1.0.0' }],
      loadArtifact: async (locator) => `phase:${locator.match(/([^/]+)\.json$/)?.[1] ?? ''}`,
...
function cleanPhases(provider: EvidenceEnvelope<ProductionProviderCanaryPayload>, auth: ProductionOperationsRolloutAuthorization) {
  return (
    ['observe-only', 'maintenance-enabled', 'dr-exercise-enabled', 'steady-state-enabled'] as const
  ).map((phase, index) => phaseEnvelope(phasePayload(phase, index, auth, provider)));
}
...
describe('continuous assurance production operations rollout gate', () => {
  ...
  it('rejects a skipped phase', async () => {
    ...
    const phases = ['observe-only', 'dr-exercise-enabled', 'steady-state-enabled'].map((phase, index) =>
      phaseEnvelope(phasePayload(phase as ProductionOperationsRolloutPhase, index, auth, provider))
    );
  ...
  it('rejects observe-only side effects', async () => {
    const provider = providerEnvelope();
    const auth = authorization(provider);
    const phases = cleanPhases(provider, auth);
    phases[0] = phaseEnvelope({
      ...phases[0].payload,
      rotationTransition: {
        authorizationId: 'bad', rotatedAtMs: BASE + 10_000,
        previousCredentialSetId: 'a', previousSigningKeyId: 'b', previousEncryptionKeyId: 'c',
        newCredentialSetId: 'd', newSigningKeyId: 'e', newEncryptionKeyId: 'f',
      },
    });
    const report = await run({ provider, auth, phases });
  ...
  it('rejects an action-name escalation in observe-only even without a side-effect receipt', async () => {
    const provider = providerEnvelope();
    const auth = authorization(provider);
    const phases = cleanPhases(provider, auth);
    const executedActions: ProductionOperationsRolloutAction[] = [
      ...phases[0].payload.executedActions,
    ...
    phases[0] = phaseEnvelope({
      ...
      executedActions,
      observedActionCount: executedActions.length,
      actionIdempotencyKeys: executedActions.map((action, index) => `observe-only:escalation:${index}:${action}`),
    });
    const report = await run({ provider, auth, phases });
    expect(report.promoteHoldThresholds.holdReasons).toContain('production-operations-rollout-action-not-allowed:observe-only');
  });
  ...
  it('requires the DR phase to exercise backup storage with the canonical digest', async () => {
    ...
    const auth = authorization(provider);
    const phases = cleanPhases(provider, auth);
    phases[2] = phaseEnvelope({
      ...phases[2].payload,
      drExercise: { ...phases[2].payload.drExercise!, sourceStorageId: 'wrong-storage' },
    });
    const report = await run({ provider, auth, phases });
  ...
  it('holds on an unresolved critical alert', async () => {
    ...
    const auth = authorization(provider);
    const phases = cleanPhases(provider, auth);
    phases[3] = phaseEnvelope({
      ...phases[3].payload,
      alerts: [{ alertId: 'crit-1', severity: 'critical', status: 'open' }],
    });
    const report = await run({ provider, auth, phases });
  ...
  it('holds on exhausted rolling error budget', async () => {
    ...
    phases[3] = phaseEnvelope({
      ...phases[3].payload,
      slo: { ...phases[3].payload.slo, failureCount: 3, allowedFailureBudget: 3, remainingFailureBudget: 0 },
    });
    const report = await run({ provider, auth, phases });
  ...
  it('rejects provider/deployment identity drift from the verified canary', async () => {
    ...
    const auth = authorization(provider);
    const phases = cleanPhases(provider, auth);
    phases[1] = phaseEnvelope({
      ...phases[1].payload,
      identity: { ...phases[1].payload.identity, accountId: 'different-account' },
    });
    const report = await run({ provider, auth, phases });
  ...
  it('rejects self-reported rollout phase evidence', async () => {
    ...
    } as EvidenceEnvelope<ProductionOperationsRolloutPhasePayload>;
    const report = await run({ provider, auth, phases });

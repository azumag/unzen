...
const BASE_ACTIONS = ['provider-health', 'provider-audit', 'primary-archive-retrieval', 'backup-archive-retrieval', 'pager-canary'] as const;
const ROLLOUT_ACTIONS = [...BASE_ACTIONS, 'credential-key-rotation', 'dr-failover-exercise'] as const;
export type ProductionOperationsRolloutPhase = typeof PHASES[number];
export type ProductionOperationsRolloutAction = typeof ROLLOUT_ACTIONS[number];
...
export interface ProductionOperationsRolloutPhasePayload {
  ...
  readonly executedActions: readonly ProductionOperationsRolloutAction[];
...
export async function runWorkersCoordinatorPublisherTaxProductionArchiveDrProviderContinuousAssuranceProductionOperationsRolloutGate(options: ProductionOperationsRolloutGateOptions) {
  ...
  const phaseSummaries: Array<{ phase: ProductionOperationsRolloutPhase; runId: string | null; effectiveEvidenceLevel: string | null; effectiveReadinessStatus: string | null }> = [];
  for (let index = 0; index < PHASES.length; index += 1) {
    ...
    phaseSummaries.push({ phase, runId: evidence.runId, effectiveEvidenceLevel: validation.effectiveEvidenceLevel ?? null, effectiveReadinessStatus: validation.effectiveReadinessStatus ?? null });
    if (evidence.evidenceKind !== PUBLISHER_TAX_EXCEPTION_ARCHIVE_DR_PROVIDER_CONTINUOUS_ASSURANCE_PRODUCTION_OPERATIONS_ROLLOUT_PHASE_EVIDENCE_KIND) reasons.push(`production-operations-rollout-phase-evidence-kind-invalid:${phase}`);
    if (!evidenceSupportsReadiness(validation, 'production-approved')) reasons.push(`production-operations-rollout-phase-evidence-not-production-approved:${phase}`);
  ...
  const obligations = options.phaseEvidences[PHASES.length - 1]?.payload.operationalObligations ?? null;
...
function validateAuthorization(auth: ProductionOperationsRolloutAuthorization, canary: ProductionProviderCanaryPayload, reasons: string[]): void {
  if (!auth.rolloutId || !auth.authorizationId || !auth.changeTicketId || !auth.rollbackControlId || !auth.emergencyHoldControlId) reasons.push('production-operations-rollout-authorization-identity-invalid');
  if (!Number.isFinite(auth.authorizedAtMs) || !Number.isFinite(auth.startsAtMs) || !Number.isFinite(auth.expiresAtMs) || auth.authorizedAtMs > auth.startsAtMs || auth.expiresAtMs <= auth.startsAtMs) reasons.push('production-operations-rollout-authorization-window-invalid');
  ...
  if (auth.providerName !== source.providerName || auth.accountId !== source.accountId || auth.primaryStorageId !== source.primaryStorageId || auth.backupStorageId !== source.backupStorageId || auth.archiveId !== source.archiveId || auth.archiveContentDigest !== source.archiveContentDigest || stable(auth.deploymentVersionIds) !== stable(source.deploymentVersionIds) || stable(auth.deploymentConfigFingerprints) !== stable(source.deploymentConfigFingerprints)) reasons.push('production-operations-rollout-provider-canary-identity-mismatch');
  ...
  if (auth.phasePlan.length !== PHASES.length) reasons.push('production-operations-rollout-phase-plan-invalid');
    ...
    for (let index = 0; index < PHASES.length; index += 1) {
      ...
      if (plan.phase !== PHASES[index] || plan.sequence !== index + 1 || !Number.isFinite(plan.startsAtMs) || !Number.isFinite(plan.expiresAtMs) || !Number.isFinite(plan.minimumObservationMs) || plan.minimumObservationMs <= 0 || !Number.isInteger(plan.maximumActions) || plan.maximumActions <= 0 || plan.startsAtMs < previousEnd || plan.expiresAtMs <= plan.startsAtMs || plan.startsAtMs < auth.startsAtMs || plan.expiresAtMs > auth.expiresAtMs) reasons.push(`production-operations-rollout-phase-plan-invalid:${PHASES[index]}`);
      previousEnd = plan.expiresAtMs;
    }
  }
  const maintenancePlan = auth.phasePlan[1];
  if (auth.maintenance.required && (!auth.maintenance.authorizationId || !Number.isFinite(auth.maintenance.rotationDueAtMs) || !auth.maintenance.previousCredentialSetId || !auth.maintenance.previousSigningKeyId || !auth.maintenance.previousEncryptionKeyId || !maintenancePlan || (auth.maintenance.rotationDueAtMs as number) < maintenancePlan.startsAtMs || (auth.maintenance.rotationDueAtMs as number) > maintenancePlan.expiresAtMs)) reasons.push('production-operations-rollout-maintenance-authorization-invalid');
  const drPlan = auth.phasePlan[2];
  if (!auth.drExercise.authorizationId || !Number.isFinite(auth.drExercise.changeWindowStartMs) || !Number.isFinite(auth.drExercise.changeWindowEndMs) || auth.drExercise.changeWindowEndMs <= auth.drExercise.changeWindowStartMs || !drPlan || auth.drExercise.changeWindowStartMs < drPlan.startsAtMs || auth.drExercise.changeWindowEndMs > drPlan.expiresAtMs) reasons.push('production-operations-rollout-dr-authorization-invalid');
}

function validatePhase(input: { payload: ProductionOperationsRolloutPhasePayload; evidence: EvidenceEnvelope<ProductionOperationsRolloutPhasePayload>; phase: ProductionOperationsRolloutPhase; index: number; plan: ProductionOperationsRolloutPhasePlan; previousCompletedAtMs: number; options: ProductionOperationsRolloutGateOptions; canary: ProductionProviderCanaryPayload; reasons: string[] }): void {
  const { payload, evidence, phase, index, plan, previousCompletedAtMs, options, canary, reasons } = input;
  if (payload.rolloutId !== options.rolloutAuthorization.rolloutId || payload.authorizationId !== options.rolloutAuthorization.authorizationId || payload.phase !== phase || payload.sequence !== index + 1 || payload.providerCanaryRunId !== options.providerCanaryEvidence.runId || payload.providerCanaryArtifactSha256 !== options.providerCanaryEvidence.artifact?.sha256) reasons.push(`production-operations-rollout-phase-identity-invalid:${phase}`);
  if (evidence.runId !== `${options.rolloutAuthorization.rolloutId}:${index + 1}:${phase}` || Date.parse(evidence.capturedAt) !== payload.capturedAtMs || payload.capturedAtMs !== payload.completedAtMs) reasons.push(`production-operations-rollout-phase-envelope-binding-invalid:${phase}`);
  ...
  if (payload.startedAtMs < plan.startsAtMs || payload.completedAtMs > plan.expiresAtMs || payload.completedAtMs < payload.startedAtMs || payload.startedAtMs < previousCompletedAtMs || payload.completedAtMs - payload.startedAtMs < plan.minimumObservationMs) reasons.push(`production-operations-rollout-phase-timeline-invalid:${phase}`);
  if (!Number.isInteger(payload.actionBudget) || payload.actionBudget <= 0 || payload.actionBudget > plan.maximumActions || !Number.isInteger(payload.observedActionCount) || payload.observedActionCount <= 0 || payload.observedActionCount > payload.actionBudget || !Number.isInteger(payload.replayCount) || payload.replayCount < 0 || payload.executedActions.length !== payload.observedActionCount || payload.actionIdempotencyKeys.length !== payload.observedActionCount || new Set(payload.actionIdempotencyKeys).size !== payload.actionIdempotencyKeys.length) reasons.push(`production-operations-rollout-phase-action-budget-invalid:${phase}`);
  validateExecutedActions(payload, phase, options.rolloutAuthorization, reasons);
  ...
  for (const invocation of payload.controlInvocations) if ((invocation.controlId !== options.rolloutAuthorization.rollbackControlId && invocation.controlId !== options.rolloutAuthorization.emergencyHoldControlId) || invocation.status === 'active') reasons.push(`production-operations-rollout-control-invocation-hold:${phase}`);
...
function validateExecutedActions(payload: ProductionOperationsRolloutPhasePayload, phase: ProductionOperationsRolloutPhase, auth: ProductionOperationsRolloutAuthorization, reasons: string[]): void {
  const allowed = new Set<ProductionOperationsRolloutAction>(BASE_ACTIONS);
  if (phase === 'maintenance-enabled' && auth.maintenance.required) allowed.add('credential-key-rotation');
  if (phase === 'dr-exercise-enabled') allowed.add('dr-failover-exercise');
  if (payload.executedActions.some((action) => !allowed.has(action))) reasons.push(`production-operations-rollout-action-not-allowed:${phase}`);
  if (phase === 'maintenance-enabled' && auth.maintenance.required && !payload.executedActions.includes('credential-key-rotation')) reasons.push('production-operations-rollout-maintenance-rotation-action-missing');
  if (phase === 'dr-exercise-enabled' && !payload.executedActions.includes('dr-failover-exercise')) reasons.push('production-operations-rollout-dr-action-missing');
}
...
function validateIdentity(identity: ProductionOperationsIdentitySnapshot, auth: ProductionOperationsRolloutAuthorization, phase: ProductionOperationsRolloutPhase, reasons: string[]): void {
  if (identity.providerName !== auth.providerName || identity.accountId !== auth.accountId || identity.primaryStorageId !== auth.primaryStorageId || identity.backupStorageId !== auth.backupStorageId || identity.archiveId !== auth.archiveId || identity.archiveContentDigest !== auth.archiveContentDigest || stable(identity.deploymentVersionIds) !== stable(auth.deploymentVersionIds) || stable(identity.deploymentConfigFingerprints) !== stable(auth.deploymentConfigFingerprints)) reasons.push(`production-operations-rollout-identity-drift:${phase}`);
}
function validateSlo(slo: ProductionOperationsSloSnapshot, phase: ProductionOperationsRolloutPhase, reasons: string[]): void {
  if (!Number.isInteger(slo.operationCount) || slo.operationCount <= 0 || !Number.isInteger(slo.failureCount) || slo.failureCount < 0 || slo.failureCount > slo.operationCount || slo.rtoBreachCount !== 0 || slo.rpoBreachCount !== 0 || slo.integrityFailureCount !== 0 || !Number.isFinite(slo.providerAvailabilityPct) || !Number.isFinite(slo.minimumProviderAvailabilityPct) || slo.providerAvailabilityPct < slo.minimumProviderAvailabilityPct || slo.minimumProviderAvailabilityPct < 0 || slo.providerAvailabilityPct > 100 || !Number.isInteger(slo.allowedFailureBudget) || slo.allowedFailureBudget <= 0 || slo.remainingFailureBudget !== slo.allowedFailureBudget - slo.failureCount || slo.remainingFailureBudget <= 0) reasons.push(`production-operations-rollout-slo-hold:${phase}`);
}
function validatePhaseSideEffects(payload: ProductionOperationsRolloutPhasePayload, phase: ProductionOperationsRolloutPhase, plan: ProductionOperationsRolloutPhasePlan, auth: ProductionOperationsRolloutAuthorization, reasons: string[]): void {
  ...
  if (phase === 'maintenance-enabled') {
    ...
    if (auth.maintenance.required) {
      ...
      if (!rotation || rotation.authorizationId !== auth.maintenance.authorizationId || rotation.rotatedAtMs < plan.startsAtMs || rotation.rotatedAtMs > plan.expiresAtMs || rotation.rotatedAtMs > (auth.maintenance.rotationDueAtMs ?? -Infinity) || rotation.previousCredentialSetId !== auth.maintenance.previousCredentialSetId || rotation.previousSigningKeyId !== auth.maintenance.previousSigningKeyId || rotation.previousEncryptionKeyId !== auth.maintenance.previousEncryptionKeyId || !rotation.newCredentialSetId || !rotation.newSigningKeyId || !rotation.newEncryptionKeyId || rotation.newCredentialSetId === rotation.previousCredentialSetId || rotation.newSigningKeyId === rotation.previousSigningKeyId || rotation.newEncryptionKeyId === rotation.previousEncryptionKeyId) reasons.push('production-operations-rollout-maintenance-rotation-invalid');
  ...
  if (phase === 'dr-exercise-enabled') {
    ...
    if (!exercise || exercise.authorizationId !== auth.drExercise.authorizationId || !exercise.exerciseId || exercise.sourceStorageId !== auth.backupStorageId || exercise.integrityStatus !== 'pass' || exercise.observedContentDigest !== auth.archiveContentDigest || exercise.completedAtMs < exercise.startedAtMs || exercise.startedAtMs < auth.drExercise.changeWindowStartMs || exercise.completedAtMs > auth.drExercise.changeWindowEndMs || exercise.startedAtMs < plan.startsAtMs || exercise.completedAtMs > plan.expiresAtMs) reasons.push('production-operations-rollout-dr-exercise-invalid');
    return;
  }
...
function validateObligations(obligations: ProductionOperationsOperationalObligations | undefined, completedAtMs: number, auth: ProductionOperationsRolloutAuthorization, reasons: string[]): void {
  if (!obligations || obligations.nextCycleDueAtMs <= completedAtMs || obligations.nextRotationDueAtMs <= completedAtMs || obligations.nextDrExerciseDueAtMs <= completedAtMs || obligations.evidenceRetentionUntilMs <= completedAtMs || !obligations.onCallRoute || !obligations.escalationTarget || obligations.rollbackControlId !== auth.rollbackControlId || obligations.emergencyHoldControlId !== auth.emergencyHoldControlId) reasons.push('production-operations-rollout-operational-obligations-invalid');
}
function stable(value: unknown): string { return JSON.stringify(sort(value)); }
function sort(value: unknown): unknown { if (Array.isArray(value)) return value.map(sort); if (value && typeof value === 'object') return Object.fromEntries(Object.keys(value as Record<string, unknown>).sort().map((key) => [key, sort((value as Record<string, unknown>)[key])])); return value; }

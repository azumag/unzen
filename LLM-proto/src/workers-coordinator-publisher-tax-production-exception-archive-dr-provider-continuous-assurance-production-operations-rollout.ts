...
export async function runWorkersCoordinatorPublisherTaxProductionArchiveDrProviderContinuousAssuranceProductionOperationsRolloutGate(
  ...
  if (options.phaseEvidences.length !== PHASES.length) {
    reasons.push('production-operations-rollout-phase-count-invalid');
  }
  ...
  for (let index = 0; index < PHASES.length; index += 1) {
    ...
    const validation = await validateEvidenceEnvelope<ProductionOperationsRolloutPhasePayload>(
      evidence,
...
function validateAuthorization(
  ...
  if (new Set(auth.approvers.filter(Boolean)).size < 2) {
    reasons.push('production-operations-rollout-two-person-approval-required');
  }
  const canaryAuth = canary.authorization;
  if (auth.providerName !== canaryAuth.providerName || auth.accountId !== canaryAuth.accountId ||
    auth.primaryStorageId !== canaryAuth.primaryStorageId || auth.backupStorageId !== canaryAuth.backupStorageId ||
    auth.archiveId !== canaryAuth.archiveId || auth.archiveContentDigest !== canaryAuth.archiveContentDigest ||
    stable(auth.deploymentVersionIds) !== stable(canaryAuth.deploymentVersionIds) ||
    stable(auth.deploymentConfigFingerprints) !== stable(canaryAuth.deploymentConfigFingerprints)) {
  ...
  if (auth.phasePlan.length !== PHASES.length) {
    reasons.push('production-operations-rollout-phase-plan-invalid');
  } else {
    let previousEnd = auth.startsAtMs;
    for (let index = 0; index < PHASES.length; index += 1) {
      const plan = auth.phasePlan[index];
      if (plan.phase !== PHASES[index] || plan.sequence !== index + 1 ||
        !Number.isFinite(plan.startsAtMs) || !Number.isFinite(plan.expiresAtMs) ||
        !Number.isFinite(plan.minimumObservationMs) || plan.minimumObservationMs <= 0 ||
        !Number.isInteger(plan.maximumActions) || plan.maximumActions <= 0 ||
        plan.startsAtMs < previousEnd || plan.expiresAtMs <= plan.startsAtMs ||
        plan.startsAtMs < auth.startsAtMs || plan.expiresAtMs > auth.expiresAtMs) {
        reasons.push(`production-operations-rollout-phase-plan-invalid:${PHASES[index]}`);
      }
      previousEnd = plan.expiresAtMs;
    }
  }
  const maintenance = auth.maintenance;
  if (maintenance.required) {
    if (!maintenance.authorizationId || !Number.isFinite(maintenance.rotationDueAtMs) ||
      !maintenance.previousCredentialSetId || !maintenance.previousSigningKeyId || !maintenance.previousEncryptionKeyId) {
      reasons.push('production-operations-rollout-maintenance-authorization-invalid');
    }
  }
  if (!auth.drExercise.authorizationId || !Number.isFinite(auth.drExercise.changeWindowStartMs) ||
    !Number.isFinite(auth.drExercise.changeWindowEndMs) ||
    auth.drExercise.changeWindowEndMs <= auth.drExercise.changeWindowStartMs) {
    reasons.push('production-operations-rollout-dr-authorization-invalid');
  }
}
...
function validatePhase(input: {
  ...
  if (payload.rolloutId !== options.rolloutAuthorization.rolloutId ||
    payload.authorizationId !== options.rolloutAuthorization.authorizationId ||
    payload.phase !== phase || payload.sequence !== index + 1 ||
    payload.providerCanaryRunId !== options.providerCanaryEvidence.runId ||
    payload.providerCanaryArtifactSha256 !== options.providerCanaryEvidence.artifact?.sha256) {
    reasons.push(`production-operations-rollout-phase-identity-invalid:${phase}`);
  }
  const expectedRunId = `${options.rolloutAuthorization.rolloutId}:${index + 1}:${phase}`;
  if (evidence.runId !== expectedRunId || Date.parse(evidence.capturedAt) !== payload.capturedAtMs ||
    payload.capturedAtMs !== payload.completedAtMs) {
    reasons.push(`production-operations-rollout-phase-envelope-binding-invalid:${phase}`);
  }
  if (options.expectedPhaseVerifierName && evidence.verification?.verifier !== options.expectedPhaseVerifierName) {
    reasons.push(`production-operations-rollout-phase-verifier-invalid:${phase}`);
  }
  if (payload.startedAtMs < plan.startsAtMs || payload.completedAtMs > plan.expiresAtMs ||
    payload.completedAtMs < payload.startedAtMs || payload.startedAtMs < previousCompletedAtMs ||
    payload.completedAtMs - payload.startedAtMs < plan.minimumObservationMs) {
    reasons.push(`production-operations-rollout-phase-timeline-invalid:${phase}`);
  }
  if (!Number.isInteger(payload.actionBudget) || payload.actionBudget <= 0 || payload.actionBudget > plan.maximumActions ||
    !Number.isInteger(payload.observedActionCount) || payload.observedActionCount <= 0 ||
    payload.observedActionCount > payload.actionBudget || !Number.isInteger(payload.replayCount) || payload.replayCount < 0 ||
    payload.actionIdempotencyKeys.length !== payload.observedActionCount ||
  ...
  if (payload.alerts.some((alert) => alert.severity === 'critical' && alert.status !== 'resolved')) {
    reasons.push(`production-operations-rollout-critical-alert-unresolved:${phase}`);
  }
  if (payload.incidents.some((incident) => (incident.severity === 'sev1' || incident.severity === 'sev2') && incident.status === 'active')) {
    reasons.push(`production-operations-rollout-active-major-incident:${phase}`);
  }
  for (const invocation of payload.controlInvocations) {
    if ((invocation.controlId !== options.rolloutAuthorization.rollbackControlId &&
      invocation.controlId !== options.rolloutAuthorization.emergencyHoldControlId) || invocation.status === 'active') {
  ...
  if (phase === 'steady-state-enabled') {
    validateObligations(payload.operationalObligations, payload.completedAtMs, options.rolloutAuthorization, reasons);
  } else if (payload.operationalObligations) {
    reasons.push(`production-operations-rollout-premature-obligations:${phase}`);
  }
  if (canary.artifactSha256 !== options.providerCanaryEvidence.artifact?.sha256) {
    reasons.push('production-operations-rollout-provider-canary-artifact-mismatch');
  }
}

function validateIdentity(
  identity: ProductionOperationsIdentitySnapshot,
  auth: ProductionOperationsRolloutAuthorization,
  phase: ProductionOperationsRolloutPhase,
  reasons: string[],
): void {
  if (identity.providerName !== auth.providerName || identity.accountId !== auth.accountId ||
    identity.primaryStorageId !== auth.primaryStorageId || identity.backupStorageId !== auth.backupStorageId ||
    identity.archiveId !== auth.archiveId || identity.archiveContentDigest !== auth.archiveContentDigest ||
    stable(identity.deploymentVersionIds) !== stable(auth.deploymentVersionIds) ||
    stable(identity.deploymentConfigFingerprints) !== stable(auth.deploymentConfigFingerprints)) {
...
function validateSlo(slo: ProductionOperationsSloSnapshot, phase: ProductionOperationsRolloutPhase, reasons: string[]): void {
  if (!Number.isInteger(slo.operationCount) || slo.operationCount <= 0 ||
    !Number.isInteger(slo.failureCount) || slo.failureCount < 0 || slo.failureCount > slo.operationCount ||
    slo.rtoBreachCount !== 0 || slo.rpoBreachCount !== 0 || slo.integrityFailureCount !== 0 ||
    !Number.isFinite(slo.providerAvailabilityPct) || !Number.isFinite(slo.minimumProviderAvailabilityPct) ||
    slo.providerAvailabilityPct < slo.minimumProviderAvailabilityPct || slo.minimumProviderAvailabilityPct < 0 ||
    slo.providerAvailabilityPct > 100 || !Number.isInteger(slo.allowedFailureBudget) || slo.allowedFailureBudget <= 0 ||
    slo.remainingFailureBudget !== slo.allowedFailureBudget - slo.failureCount || slo.remainingFailureBudget <= 0) {
    reasons.push(`production-operations-rollout-slo-hold:${phase}`);
  }
}

function validatePhaseSideEffects(
  payload: ProductionOperationsRolloutPhasePayload,
  phase: ProductionOperationsRolloutPhase,
  plan: ProductionOperationsRolloutPhasePlan,
  auth: ProductionOperationsRolloutAuthorization,
  ...
  if (phase === 'maintenance-enabled') {
    ...
    if (auth.maintenance.required) {
      ...
      if (!rotation || rotation.authorizationId !== auth.maintenance.authorizationId ||
        rotation.rotatedAtMs < plan.startsAtMs || rotation.rotatedAtMs > plan.expiresAtMs ||
        rotation.rotatedAtMs > (auth.maintenance.rotationDueAtMs ?? -Infinity) ||
        rotation.previousCredentialSetId !== auth.maintenance.previousCredentialSetId ||
        rotation.previousSigningKeyId !== auth.maintenance.previousSigningKeyId ||
        rotation.previousEncryptionKeyId !== auth.maintenance.previousEncryptionKeyId ||
        !rotation.newCredentialSetId || !rotation.newSigningKeyId || !rotation.newEncryptionKeyId ||
        rotation.newCredentialSetId === rotation.previousCredentialSetId ||
        rotation.newSigningKeyId === rotation.previousSigningKeyId ||
        rotation.newEncryptionKeyId === rotation.previousEncryptionKeyId) {
        reasons.push('production-operations-rollout-maintenance-rotation-invalid');
      }
    } else if (payload.rotationTransition) {
      reasons.push('production-operations-rollout-unexpected-rotation');
    }
    return;
  }
  if (phase === 'dr-exercise-enabled') {
    ...
    if (!exercise || exercise.authorizationId !== auth.drExercise.authorizationId || !exercise.exerciseId ||
      exercise.sourceStorageId !== auth.backupStorageId || exercise.integrityStatus !== 'pass' ||
      exercise.observedContentDigest !== auth.archiveContentDigest || exercise.completedAtMs < exercise.startedAtMs ||
      exercise.startedAtMs < auth.drExercise.changeWindowStartMs || exercise.completedAtMs > auth.drExercise.changeWindowEndMs ||
      exercise.startedAtMs < plan.startsAtMs || exercise.completedAtMs > plan.expiresAtMs) {
      reasons.push('production-operations-rollout-dr-exercise-invalid');
    }
    return;
  }
  if (payload.rotationTransition || payload.drExercise) {
    reasons.push('production-operations-rollout-steady-state-promotion-side-effect-detected');
  }
}

function validateObligations(
  obligations: ProductionOperationsOperationalObligations | undefined,
  completedAtMs: number,
  auth: ProductionOperationsRolloutAuthorization,
  reasons: string[],
): void {
  if (!obligations || obligations.nextCycleDueAtMs <= completedAtMs || obligations.nextRotationDueAtMs <= completedAtMs ||
    obligations.nextDrExerciseDueAtMs <= completedAtMs || obligations.evidenceRetentionUntilMs <= completedAtMs ||
    !obligations.onCallRoute || !obligations.escalationTarget ||
    obligations.rollbackControlId !== auth.rollbackControlId || obligations.emergencyHoldControlId !== auth.emergencyHoldControlId) {
    reasons.push('production-operations-rollout-operational-obligations-invalid');
  }
}
...
function sort(value: unknown): unknown {
  ...
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.keys(value as Record<string, unknown>).sort().map((key) => [
      key,
      sort((value as Record<string, unknown>)[key]),
    ]));
  }
  return value;
}

...
The probe report also carries the pinned external-data identity (`model_q4.onnx_data`, 1,692,672,000 bytes, SHA-256 `07cc629ef2cb7fdb18615ce2e4f3774f763e6fc840207d772a8b511eead36647`). The CLI requires the blueprint location and local source byte size to match that identity, then computes the complete-source SHA-256 while emitting every selected source range during the same sequential read. A pinned-source hash mismatch still fails closed and removes payloads created by that run, so the optimization does not allow a different file with the same basename to become accepted evidence. For the preferred four-payload blueprint this reduces producer-side source reads from 1,692,672,000 + 1,050,673,152 bytes (about 2.74 GB) to exactly one 1,692,672,000-byte pass.

The materializer also validates that row coverage and source byte ranges are contiguous, rejects unsafe source locations and truncated source files, refuses to overwrite existing payloads or reports, writes each output slice with bounded memory, and records the actual byte count and SHA-256 of every emitted slice. The `payload-*.bin` basename namespace is reserved inside the payload directory even for filenames outside the current expected numeric set: materialization starts only when that namespace is empty, re-enumerates it immediately before returning a pass report, and removes only payloads created by the current run if a late extra entry appears. `--report-out` likewise cannot create a post-materialization entry that a later exact-payload-set verification would interpret as an extra payload, including indirectly through a nested path such as `payload-9999.bin/report.json` whose parent creation would itself occupy the reserved namespace. It also rejects a report file path that is the payload directory itself or an ancestor of it before materialization starts; those layouts are structurally impossible because the same path would need to be both a report file and a directory, and previously could waste the full pinned source pass before failing. The producer validates the report path before the potentially long source pass, revalidates it after materialization, creates any missing report parent directories, and then resolves and pins the validated parent directory to an open descriptor before exclusive file creation. The requested parent identity is rechecked against that descriptor before and after the write, while the child is created relative to the pinned descriptor with exclusive/no-follow semantics; late alias drift therefore fails closed, and a report created by this run is removed only if its descriptor-pinned inode still owns the child name. The pinned source is opened once, identified by device/inode/size/mtime/ctime, and consumed through that same file descriptor for both the full hash and payload emission; the descriptor snapshot and source path are rechecked before a pass report is returned. Each emitted payload is likewise pinned to the device/inode/size/mtime/ctime observed on its write descriptor and rechecked after creation and again before the materialization report object is returned, so same-size mutation, deletion, symlink substitution, or path replacement fails closed. Cleanup checks that a path still points to the inode created by the current run before unlinking it; an already-observed foreign path replacement is deliberately left untouched. If this run created the payload directory and later fails, it removes that directory only while the path still resolves to the same created directory identity, so a directory created or substituted by another process is not removed. Before payload children are created, the materializer also pins the payload directory itself to an open directory descriptor; payload creation, reserved-namespace rechecks, snapshot rechecks, and cleanup are performed relative to that descriptor while the requested directory path identity is rechecked. Renaming or replacing the payload directory therefore cannot redirect a long-running materialization into a different directory, and failure cleanup still targets only the descriptor-pinned files created by this run. Unit tests use a tiny synthetic external-data file; CI still does not download the 1.7 GB pinned weight file, so the real full-file identity check remains a local evidence step when that file is already available.
...
`verify_endpoint_payload_materialization.py` verifies the producer evidence as a separate, source-backed pass. It requires the pinned external-data file plus the expected `--stage` and `--tier`, independently derives the pinned blueprint/provenance from the probe report, and computes the complete-source SHA-256 plus every referenced source-range SHA-256 in one sequential read of the same pinned file snapshot before independently re-hashing every payload. It also checks source coverage and payload geometry/count/total bytes, requires the payload directory to contain exactly the expected `payload-*.bin` set both before hashing and immediately before a pass report is returned, and rejects symlink payload files. Source symlinks remain supported for model-cache compatibility because the dereferenced bytes are independently checked against the pinned size/SHA-256 and per-range digests.
...
The explicit stage/tier arguments mean the verifier does not trust those claims from the producer report. More importantly, a forged or corrupted payload plus a correspondingly edited producer hash still fails unless the payload digest matches the exact byte range in the pinned source file. The verifier also hashes the exact UTF-8 JSON bytes it parses for both input reports and includes those digests in `inputReports`, so the result is tied to the precise probe/materialization documents. Its `--report-out` path is checked before the expensive verification pass and again after verification. After parent-directory creation, the verifier independently resolves and pins the validated parent directory to an open descriptor, rechecks the requested parent identity before and after the write, and creates the report child relative to that descriptor with exclusive/no-follow semantics. Any path that would use a direct-child `payload-*.bin` entry or create descendants beneath one is rejected, while alias drift through the final create/write boundary fails closed without deleting a foreign replacement. Its own report remains `decisionStatus=diagnostic-only`: independent source-backed verification strengthens the evidence chain but does not select vocabulary-axis chunking or change browser cache, manifest, loader, runtime, dispatcher, or artifact-policy semantics.

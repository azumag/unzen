...
Regular files are opened with no-follow semantics where the host provides them and are re-statted through the open file descriptor. The descriptor's device/inode identity must match the non-symlink regular file observed by the pre-open `lstat()`, so a regular-file pathname rebind before `open()` fails before any bytes are consumed. Large graph/payload files are hashed through a bounded 1 MiB buffer rather than being loaded into memory as one `Buffer`. After streaming, the descriptor's device/inode, size, `mtime`, and `ctime` must still match the accepted pre-read snapshot, and the pathname must still identify the same non-symlink regular file. `atime` is intentionally excluded because hashing can update it. Observable same-size rewrites, pathname disappearance, symlink replacement, and inode replacement therefore fail closed before the hash result is accepted.

The hashing snapshot check preserves the accepted descriptor as the byte source but is not an adversarial filesystem snapshot primitive: a filesystem that changes content without exposing any device/inode/size/mtime/ctime change can still evade the metadata guard.

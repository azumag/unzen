...
The two bounded source-graph readers that still keep caller-specific snapshot logic rather than delegating to the shared helper follow the same raw-byte rule: `tools/multi_segment_onnx.py::_read_source_graph_snapshot()` and `tools/diagnose_multi_segment_budget.py::_read_source_graph_snapshot()` also request `O_BINARY` when the platform exposes it. Their existing path/descriptor identity checks and byte ceilings remain unchanged.

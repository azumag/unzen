...
Generated multi-segment manifest creation is also bound to that captured workspace generation. Before copying the verified split manifest, `artifact_execution_snapshot.py` opens the workspace itself with `O_DIRECTORY|O_NOFOLLOW`, validates the opened descriptor against the captured `(st_dev, st_ino)`, and creates the manifest relative to that descriptor with exclusive-create semantics. This applies even when full nested component walking is unavailable and artifact hard-link pinning must use the safe pathname fallback. A replacement installed at the workspace pathname after validation therefore cannot receive the copied manifest; if replacement occurs after the directory descriptor is opened, the manifest is written only into the already-accepted generation and later pathname checks fail closed.
...
The deterministic race regressions cover both ends of the workspace lifecycle: manifest creation is raced after the accepted root descriptor is opened, and cleanup is raced after its descriptor has been accepted but before recursive deletion begins. They verify that snapshot content and recursive deletion stay bound to the accepted generation while replacement contents remain untouched.

...
Issue #397 extends the portability contract to Windows alternate-data-stream syntax. At the current implementation stage, the split producer `multi_segment_onnx.py`, `verify_multi_segment_artifacts.py`, `verify_multi_segment_capture_source.py`, `verify_multi_segment_capture_source_provenance.py`, and `verify_multi_segment_onnx.py` reject any path containing `:` in a Windows path component. The remaining independent verifier/auditor boundaries stay tracked by #397 and must not be described as hardened until their own checks and regressions land.

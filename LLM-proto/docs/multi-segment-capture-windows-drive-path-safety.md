...
Issue #406 extends the contract to Windows reserved-device and trailing-dot/space aliases. Propagation is intentionally staged so each independent boundary receives focused regression coverage; `verify_multi_segment_artifacts.py`, `verify_multi_segment_capture_source_provenance.py`, and `verify_multi_segment_capture_source.py` are covered so far.

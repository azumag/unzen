...
Issue #388 additionally hardens the rooted-but-drive-less edge case. The split producer, `verify_multi_segment_artifacts.py`, `verify_multi_segment_capture_source_provenance.py`, `verify_multi_segment_onnx.py`, `audit_multi_segment_capture.py`, and `verify_multi_segment_capture_bundle.py` now reject relevant locations with either a non-empty Windows `drive` or `root`. The same `root` check still needs to be propagated through the remaining independent downstream validator boundaries before #388 is complete:

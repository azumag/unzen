...
Issue #388 additionally hardens the rooted-but-drive-less edge case. The split producer, `verify_multi_segment_artifacts.py`, and `verify_multi_segment_capture_source_provenance.py` now reject relevant locations with either a non-empty Windows `drive` or `root`. The same `root` check still needs to be propagated through the remaining independent downstream validator/auditor boundaries before #388 is complete:

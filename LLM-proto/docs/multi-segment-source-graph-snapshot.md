...
The generator now captures the source graph once before planning. The snapshot reader resolves the requested path, requires the resolved target to be a regular file, opens it read-only with nonblocking/no-follow flags where supported, and caps the graph at 64 MiB. The shared `read_regular_file_snapshot()` byte ceiling must be a positive Python `int`; booleans, floats (including `NaN` and infinities), zero, and negative values are rejected before path resolution or filesystem I/O so malformed numeric input cannot disable or distort the bound. It rejects path-to-open identity changes, read-time mutation or growth, resolved-target replacement, and requested-path retargeting. Stable symlinks remain accepted so existing source-path semantics are preserved.

...
- verifies the requested pathname still resolves to a non-symlink regular file with the same device/inode/size/mtime/ctime snapshot after the read;
- only then returns validated UTF-8 text for JSON parsing and the existing captured-evidence validators.

This closes local pathname-replacement and observable final-window metadata drift in offline verification while bounding the memory consumed before JSON parsing. A verifier should not report success for bytes read from one snapshot while the reviewed evidence pathname has moved to another inode or still names the same inode with an observably different size/mtime/ctime before the final path validation. A malformed or accidentally huge evidence pathname must not cause an unbounded read, and invalid UTF-8 must fail closed rather than being normalized into different text before JSON parsing. `atime` is deliberately excluded because reading can update it.
...
This is a filesystem snapshot/read-integrity and text-decoding check, not a signature or same-user adversary boundary. A process with sufficient local privileges can still modify files before or after the bounded verification window, and a filesystem that changes content without exposing any device/inode/size/mtime/ctime change can evade the metadata guard. Content/source/Chrome/WebGPU/tile/numerical validation remains the responsibility of the existing captured-evidence contract.

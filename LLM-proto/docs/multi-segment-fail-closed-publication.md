...
3. Re-run the existing source-collision and destination-node preflights against the *final* output paths. This preserves the source overwrite, symlink, non-regular node, and multiple-hard-link protections added to the base generator.
4. Verify that every staged graph/external-data file named by the generated layout exists.
5. Remove the previously published `split-manifest.json` before replacing the first final artifact.
6. Move graph and external-data files into place. If a current segment is fully embedded, remove a stale previous `segmentN.onnx_data` for that segment.

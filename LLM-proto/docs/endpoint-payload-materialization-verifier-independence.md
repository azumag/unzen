...
The verifier still performs the source-backed checks introduced previously: complete source size/SHA-256, exact source-range hashes, exact payload hashes, report geometry, payload count and total bytes, exact `payload-*.bin` set, provenance, and input-report byte hashes. Source symlinks are allowed only because the dereferenced bytes are rehashed against the pinned identity; payload symlinks are rejected.

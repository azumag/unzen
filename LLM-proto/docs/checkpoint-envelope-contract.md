...
The hidden-state validation checks the platform's `ArrayBuffer.isView()` authority before it reads typed-array internal-slot properties such as `byteLength` or takes a `.slice()` copy. A `Proxy` around a `Uint8Array` can satisfy `instanceof Uint8Array` while lacking those internal slots, so Proxy-backed payloads are rejected with the checkpoint domain validation error rather than leaking a native `TypeError`. Genuine `Uint8Array` instances and subclasses retain the existing behavior.

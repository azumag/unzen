...
The objective ordering is regression-checked independently of the dynamic-programming implementation. `test_multi_segment_partition_exhaustive_oracle.py` enumerates every contiguous partition for bounded synthetic models, including deliberately non-monotonic span-cost tables, and compares the exact DP cuts and costs with the exhaustive objective tuple. In infeasible cases it also checks the globally minimum achievable maximum and oversized singleton diagnostics. This test is host-side planner correctness evidence only; it does not replace real 1B/q4 artifact or browser execution evidence.

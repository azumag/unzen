...
The router creates a plain, frozen snapshot from the validated fields. Later caller mutation therefore cannot change VRAM accounting or geometry. Malformed metadata fails before manifest-backed residency compatibility logic, so values such as `Symbol` cannot reach string operations such as hash normalization.

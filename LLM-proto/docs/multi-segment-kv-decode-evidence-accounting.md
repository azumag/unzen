...
The verifier treats the JSON as untrusted input. It rejects symlinks and non-regular files, limits the input to 16 MiB by default, uses `O_NOFOLLOW` where the platform provides it, and compares device/inode/size/mtime/ctime before and after the read so replacement or mutation during verification fails closed. On platforms exposing `O_BINARY`, the evidence regular-file descriptor also requests binary mode so byte counts and the returned input stream remain the exact persisted bytes rather than CRT text-mode translated bytes; POSIX behavior is unchanged. The limit can be changed with a positive integer `UNZEN_KV_DECODE_EVIDENCE_MAX_BYTES` value.

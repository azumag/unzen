...
The lower-level `materialize_external_data()` helper is also a direct programmatic trust boundary. It validates `mode` before inspecting locations, then preflights the complete location sequence before source existence checks or destination mutation. Every location must be a non-empty string, relative, and free of `..` path components. Therefore an unsupported mode or a malformed later location cannot create directories, unlink destinations, or partially materialize an earlier valid external-data file. Safe nested relative locations retain the existing `copy`, `symlink`, and `none` behavior.

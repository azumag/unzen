...
The lower-level `split_llama_1b_onnx.sha256_file()` helper applies the same runtime-input rule to its bounded read control. `chunk_size` must be an exact positive Python integer, with booleans rejected even though they subclass `int`; zero, negative values, floats (including `NaN` and infinities), strings, `None`, and other non-integers fail before `Path.open()`. Valid positive chunk sizes, including the default, continue to hash the complete file. This prevents `read(0)` from silently producing the empty-payload digest and prevents negative values such as `-1` from turning a bounded hash loop into an unbounded whole-file read.

The lower-level `materialize_external_data()` helper is also a direct programmatic trust boundary. It validates `mode` before inspecting locations, then preflights the complete location sequence before source filesystem checks or destination mutation. Every location must be a non-empty string, relative, and free of `..` path components. For `copy` and `symlink`, it verifies that every referenced source both exists and resolves to a regular file, then preflights every destination and required parent before creating directories, unlinking destinations, copying, or creating destination symlinks. A later destination that is already a real directory, or a later required parent that is a file/broken symlink rather than a directory, therefore fails before any earlier destination is replaced. Existing regular-file and symlink destinations retain their replacement behavior, and a symlink whose source target is a regular file remains valid because `Path.is_file()` follows the source symlink.

Destination containment follows the compatibility-preserving fail-closed policy selected in #1099: the output directory itself defines the canonical root after resolving any symlinks in that root path, and an existing symlinked destination parent is allowed only when its resolved target remains inside that canonical root. An intermediate symlink that escapes the root rejects the entire plan before any destination mutation, while an in-root symlinked parent remains supported. This is a preflight guarantee for the local tooling boundary rather than filesystem-level race isolation; callers must not concurrently rewrite destination-directory symlinks while materialization is running. `none` deliberately stops after location preflight and performs no source or destination lookup. Safe nested relative locations retain the existing `copy`, `symlink`, and `none` behavior.

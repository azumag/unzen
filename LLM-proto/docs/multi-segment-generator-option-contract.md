...
The lower-level `materialize_external_data()` helper is also a direct programmatic trust boundary. It validates `mode` before inspecting locations, then preflights the complete location sequence before source existence checks or destination mutation. Every location must be a non-empty string, relative, and free of `..` path components. For `copy` and `symlink`, it then verifies that every referenced source exists before creating directories, unlinking destinations, copying, or creating symlinks; therefore a missing later source cannot partially replace an earlier destination. `none` deliberately stops after location preflight and performs no source lookup. Safe nested relative locations retain the existing `copy`, `symlink`, and `none` behavior.

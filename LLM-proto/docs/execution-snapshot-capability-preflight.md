...
Capability predicates treat missing required OS callables as unsupported rather than dereferencing them while probing. In particular, a reduced Python host without `os.link` cannot advertise no-follow hard-link or component-anchored support, and a host without `os.stat` cannot advertise no-follow stat or component-anchored support. The anchored predicate also rejects hosts missing the other operations it directly relies on (`os.open`, `os.mkdir`, `os.unlink`, `os.fstat`, or `os.close`). Generation-bound cleanup independently requires `os.open`, `os.stat`, `os.fstat`, `os.scandir`, `os.unlink`, `os.rmdir`, and `os.close`, descriptor-relative support for the relevant operations, descriptor-backed `scandir`, `os.stat(..., follow_symlinks=False)`, and `O_DIRECTORY|O_NOFOLLOW`. The generated snapshot additionally requires descriptor-relative `os.open`, callable `os.write`/`os.close`, real integer `O_WRONLY`/`O_CREAT`/`O_EXCL`, and rejects malformed present `O_NOFOLLOW`/`O_CLOEXEC` placeholders. This keeps the preflight machine-readable and fail-closed instead of leaking `AttributeError`/`TypeError` from capability detection, cleanup, or manifest creation.

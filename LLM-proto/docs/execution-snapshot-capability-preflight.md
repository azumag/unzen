...
Generated multi-segment snapshots have one additional requirement that source-model and legacy snapshots do not: the accepted manifest bytes are copied into the temporary execution tree before the generated graph/external-data hard links are exposed to ORT. That copy requires `snapshotManifestWrite=true`. `O_NOFOLLOW` and `O_CLOEXEC` remain opportunistic flags exactly as in the manifest-write implementation; they are not reported as manifest-write blockers because `_snapshot_create_flags()` already treats them as optional.

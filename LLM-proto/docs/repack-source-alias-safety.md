...
The same preflight parses all external-data ranges, resolves and opens every unique source file for reading, and validates each declared `offset + length` against the opened file size before any destination write. The lexical source path is then mapped to that already-open descriptor for the whole copy; the copy loop does not re-resolve the path. Retargeting a source symlink after preflight therefore cannot redirect or invalidate the active source stream. Missing, unreadable, malformed, or truncated source inputs fail without creating a new destination payload or truncating an existing one. Source descriptors are kept open for the copy and closed on both success and failure.

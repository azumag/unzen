...
Capture also treats the artifact-integrity verifier result as an untrusted runtime boundary before numerical work. A passing report must carry `segmentCount`, `maximumSegmentArtifactBytes`, and `effectiveRequiredMaxBytes` as exact positive Python integers. Booleans are rejected explicitly even though Python makes `bool` a subclass of `int`. This keeps mocked, programmatic, or future verifier changes from turning boolean metadata into measured count/byte values before evidence collection or publication.

This prevents an unavailable provider, malformed token/cache-shape parameters, non-finite tolerance, invalid shard budget, or malformed artifact-integrity metadata from consuming a real 1B numerical capture before failing. It does not download a model, relax artifact limits, deploy production infrastructure, or provide new physical WebGPU / real multi-browser evidence by itself.

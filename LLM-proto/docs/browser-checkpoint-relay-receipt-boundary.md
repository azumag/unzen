...
The existing semantic checks remain authoritative after parsing: segment 0 binds the receipt to the loaded manifest and segment 1 requires confirmed browser-profile isolation plus the exact checkpoint ID/digest it consumed. Abort checks around both response paths are unchanged.

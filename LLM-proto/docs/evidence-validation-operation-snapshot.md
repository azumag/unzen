...
The runtime callback references are operation-scoped as well. The validator reads `loadArtifact` once, then captures `verifyArtifact` before invoking the loader. An accessor-backed callback cannot substitute a different function on a later read, and a loader cannot replace the verifier that the already-started validation operation will use. This snapshot is limited to callback identity; it is not a general deep freeze or canonicalization of every `EvidenceValidationOptions` field.

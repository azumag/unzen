...
`tools/multi_segment_onnx.py` is the underlying #167 planner/generator. It:
...
For final or repeated real-1B artifact publication, use the staged wrapper so a
failed regeneration cannot leave an old manifest over partially replaced
artifacts:
...
The wrapper runs `multi_segment_onnx.py` in a same-filesystem staging directory,
rechecks final destination safety, removes the old manifest before the first
artifact replacement, and publishes the new `split-manifest.json` last. See
`multi-segment-fail-closed-publication.md` for the exact failure/concurrency
boundary. Direct generator use remains available for compatibility and callers
that intentionally own destination lifecycle themselves.

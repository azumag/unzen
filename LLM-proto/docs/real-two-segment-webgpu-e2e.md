...
`tools/multi_segment_onnx.py` plans contiguous decoder-layer spans for the 1B
scale-up before writing artifacts. It evaluates dependency-closed graph spans,
counts every external initializer referenced by each span (including shared
initializers), and chooses the smallest segment count that keeps every shard
inside `--preferred-max-bytes`. Among partitions with that segment count, it
picks the one closest to `--target-bytes`. The generated manifest records cut
layers, boundary tensors, estimates, real artifact bytes, hashes, and budget
verdicts; the final gate always uses generated bytes, not estimates.
...
The planner is preparation-only evidence. Numerical equivalence, Coordinator
relay, cache behavior, and WebGPU execution still require a real-model run and
must be recorded separately.

#158 remains HOLD until the real P0 above succeeds.

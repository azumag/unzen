...
The full source verifier applies the same raw-byte rule to the original model graph and external-data payloads. Regular-file descriptors that feed source SHA-256 hashing request `O_BINARY` where the platform exposes it, while directory anchors and component-walk directory descriptors remain directory-only opens. This keeps the reported `st_size` and SHA-256 bound to the exact persisted source bytes on Windows without changing POSIX behavior or weakening the existing no-follow, identity, and mutation checks.

...
3. The requested model's parent directory is resolved once immediately before workspace creation. The temporary tree is created through that resolved parent, and the resulting workspace path is resolved before it is retained for all later snapshot writes and cleanup. A later retarget of a caller-visible parent symlink therefore cannot redirect the workspace or its removal.

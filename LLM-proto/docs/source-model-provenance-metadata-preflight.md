...
- external-data locations are globally unique;
...
If any later entry is malformed, the verifier fails before hashing the source graph or any earlier external-data payload. This is a fail-fast performance and trust-boundary guarantee; it does not weaken the measured identity checks.
...
The emitted `sourceModel` verification report is unchanged for valid manifests: it records graph byte size/digest, each external-data location/byte size/digest, and `allExternalDataHashed: true`.

## Scope

This hardening is host-side numerical-evidence reliability for #167 / #943. It does not change ONNX Runtime provider policy, browser artifact layout, production deployment, credentials, billing, or count as new real-browser/WebGPU evidence.

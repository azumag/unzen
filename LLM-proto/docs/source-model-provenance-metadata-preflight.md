...
For the source graph, metadata preflight now preserves the target resolved from the lexical `--full-model` path and passes it to `_measure_file()` as `expected_resolved`. A stable symlinked `--full-model` remains supported, but retargeting that symlink or any path component after path/object preflight and before descriptor open fails closed even when the replacement has identical bytes and SHA-256. The existing descriptor-pinned final path recheck continues to reject retargeting after open.

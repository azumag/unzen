...
The source verifier performs its own bundle verification as well, so the combined command observes the capture twice. The final report is emitted only when the immutable manifest digest, source graph digest, and capture status remain identical across those measurements.

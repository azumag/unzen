...
Only after the lightweight host probe, Chrome-identity binding, and default-device diagnostic are complete does the preflight stream every required ONNX graph and physical payload through SHA-256 verification. It rejects symlinked artifacts, non-regular files, byte-length drift, digest drift, or a file whose identity changes while it is being hashed. The payloads are streamed instead of being read into one large Node.js buffer. The final `verifiedBytes` total is accumulated with a pre-addition safe-integer guard, so the report never publishes a rounded aggregate if a future prepared-file set would cross JavaScript's exact integer range.

...
- caller-owned options are not spread or enumerated; only `maxRetries`, `perSegmentTimeoutMs`, `retryDelayMs`, and `artifactResidencyLedger` are read;
- each declared field is read at most once, so valid-first/altered-second accessors cannot drift between validation and retained state;
...
- defaults are applied only when the corresponding captured numeric field is `undefined`;
...
- `artifactResidencyLedger`, when supplied, retains its original object identity and `assertCompatibleSegments()` still runs only after the numeric envelope is valid;

...
- ASCII case-only aliases such as `weights/Chunk.bin` and `weights/chunk.bin`, even when the verifier runs on a case-sensitive POSIX filesystem; and
- any graph or external-data declaration containing a backslash, keeping the Python preflight on the same forward-slash-only artifact grammar as the generated-ONNX runtime importer.

The portable identity check ASCII-folds the resolved artifact path before comparing declarations. It deliberately does not perform locale-sensitive or general Unicode case normalization: the boundary being mirrored is the ASCII case-insensitivity relevant to ordinary generated artifact names on common Windows filesystems and to the generated-ONNX runtime importer. Backslashes are rejected before resolution rather than normalized, so a manifest accepted by this preflight cannot rely on a POSIX literal-backslash filename that the runtime would later interpret or reject differently.

All declared graph/external-data paths are preflighted before the first artifact payload is measured. A duplicate, portable case alias, backslash path, or other unsafe path in a later segment therefore fails without hashing earlier large artifacts. The same fail-fast phase now validates immutable metadata shape for every segment and browser-budget entry: canonical graph/external-data SHA-256 values, non-negative declared byte counts, non-empty tier strings, expected budget-entry indices, and aggregate declared maxima. A malformed declaration in a later segment therefore also fails before any graph or external-data payload is stream-hashed.
...
This keeps the measured `browserArtifactBytes` ledger aligned with the manifest's logical browser-cache units. A manifest cannot count one resolved file, or two portable case aliases of one logical Windows path, as independent artifact components and still pass the standalone integrity gate or the numerical verifiers that invoke that gate.

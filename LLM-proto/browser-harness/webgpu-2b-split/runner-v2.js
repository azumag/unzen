...
function externalDataForSegment(segment) {
  const entries = segment.externalData ?? [];
  if (!Array.isArray(entries) || entries.length === 0) {
    throw new Error(`segment ${segment.index} has no browser externalData manifest`);
  }
  return entries.map((entry) => ({
    // ONNX Runtime requires path to exactly match TensorProto.external_data.location.
    path: entry.location,
    data: modelUrl(entry.location),
  }));
}
...
async function createWebGpuSession(segment, manifest) {
  ...
  return ort.InferenceSession.create(modelUrl(segment.path), {
    executionProviders: ['webgpu'],
    graphOptimizationLevel: 'all',
    externalData: externalDataForSegment(segment),
  });
}
...
async function runSegment0(manifest) {
  ...
  const session = await createWebGpuSession(segment, manifest);

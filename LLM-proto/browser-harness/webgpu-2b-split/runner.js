...
async function loadManifest() {
  const response = await fetch(`${splitRoot}/split-manifest.json`, { cache: 'no-store' });
...
function normalizeTokenIds(encoded) {
  ...
  if (!Array.isArray(values)) {
    throw new Error('tokenizer did not return input_ids array');
  }
...
function resolveDimension(value, { name, axis, sequenceLength }) {
  ...
  if (text.includes('head_size') || text.includes('head_dim') || (axis === 3 && /past|key_values/.test(name))) return headSize;
  if (text.includes('sequence') || text.includes('seq')) {
    return /past|key_values/.test(name) ? 0 : sequenceLength;
  }
...
async function runSegment0(manifest) {
  ...
  const session = await createWebGpuSession(`${splitRoot}/${segment.path}`);
  const feeds = makeFeeds(session, tokenIds);
  const boundaryNames = manifest.boundary.tensors.map((entry) => entry.name);
  const started = performance.now();
  const outputs = await session.run(feeds, boundaryNames);
  ...
  status('Segment 0 completed and checkpoint is stored by Coordinator.');
}
...
async function runSegment1(manifest) {
  ...
  const session = await createWebGpuSession(`${splitRoot}/${segment.path}`);
  const feeds = makeFeeds(session, tokenIds, boundary);
  const started = performance.now();
  const outputs = await session.run(feeds, [manifest.logitsOutput]);
  const executionMs = performance.now() - started;
  ...
  const boundaryBytes = checkpoint.tensors.reduce((sum, tensor) => sum + Number(tensor.bytes), 0);
  ...
  status(`Split inference completed. next token=${JSON.stringify(tokenText)} (${top1.tokenId})`);
}
...
async function execute() {
  ...
  try {
    ...
    if (role === 'segment0') {
      await runSegment0(manifest);
    } else if (role === 'segment1' || role === 'standby') {
      await runSegment1(manifest);
    } else {
      throw new Error(`unsupported role in runner: ${role}`);
    }
  } catch (error) {
    console.error(error);
...
status('Ready. Open a second browser/profile with the same run ID.');

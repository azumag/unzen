...
Loader error reporting must not call coercion hooks on arbitrary object or function failures. `normalizeWebpackLoaderError()` therefore preserves genuine `Error` instances, preserves useful primitive rejection text, and replaces object/function failures with a stable generic `Error` without invoking `String(value)`, `Symbol.toPrimitive`, `valueOf`, or `toString` on the thrown value. The initial `instanceof Error` classification is itself guarded because a revoked or hostile Proxy can throw from `[[GetPrototypeOf]]` during `instanceof`.

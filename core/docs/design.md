...
- **自己消費**: 訪問者は自分が必要とする機能だけを実行する。他人のための計算はない
- **透過性**: 開発者は関数を定義するだけ。実行場所の切り替えはフレームワークが自動で行う
- **安全性**: QuickJSサンドボックスにより、外部接続・DOM操作・副作用を完全に遮断
...
- Wasmにコンパイルされた軽量JSエンジン (Bellard作)
- 任意のJavaScript関数を実行可能
- サンドボックスにより安全性を担保
- バイナリサイズ: ~505KB (gzip: ~150KB)
- 性能: V8 JIT比で低速だが、50ms以内の短時間関数には十分
  - V8はJITコンパイルにより高速だが、QuickJSはインタプリタ実行
  - 短時間関数 (バリデーション、フィルタリング等) では体感差なし
  - 起動が高速 (コールドスタート ~10-25ms) なのが利点
- メモリ: ベースライン ~2-4MB (V8の ~30MB と比較して軽量)

#### MoonBit ランタイム (高性能計算用)

- MoonBit言語で書かれた関数を直接Wasmにコンパイル
- wasm-gc バックエンドにより、GCはブラウザVMに委譲 (バイナリにGCランタイム不含)
- QuickJSより大幅に高速 (Rustの~10%以内、ネイティブJSの数倍)
  - ベンダーベンチマーク: fib(46) でRust 160ms vs MoonBit 177ms
  - イテレータ処理で plain JS の ~7.7倍
  - ※ ベンダー提供の数値であり、独立ベンチマークは未確認
- バイナリサイズ: 極小 (fib関数: 253bytes, HTTP component: 27KB)
- ブラウザ要件: wasm-gc 対応 (Chrome 119+, Firefox 120+, Safari 18.2+)
- Wasmサンドボックスにより安全性を担保 (メモリ隔離、外部アクセス不可)
- 用途: 数値計算、暗号処理、データ変換など計算集約型タスク
...
キャッシュ戦略:
1. **マニフェスト**: ETag/Last-Modified ヘッダで条件付きリクエスト
2. **関数コード/Wasm**: ハッシュベースの URL でイミュータブルキャッシュ (Cache-Control: immutable)
3. **Service Worker**: マニフェストを定期チェック → ハッシュ変更時のみコードを再取得
4. **更新フロー**: サーバー側で関数を変更 → ハッシュが変わる → 次回マニフェスト取得時に検出
...
4層の隔離:
1. **Web Worker**: DOM・メインスレッドから隔離
2. **WebAssembly**: メモリ空間を完全に分離
3. **QuickJS**: JS実行環境をホストJSから隔離
4. **API制限**: 危険なAPIを初期化時に削除
...
- **Wasmレベルの隔離**: メモリ空間が完全に分離 (QuickJSと同等)
- **外部接続不可**: Wasm仕様によりネットワークアクセス不可
- **インポート制限**: ホストから提供する関数を最小限に制限
...
- **デフォルト**: MoonBit関数はフォールバックなし。ブラウザ実行に失敗した場合は `UnzenRuntimeError` を送出
...
wasm-gc 未対応ブラウザ (2024年以前) ではフォールバック用JS実装がないMoonBit関数は利用不可。

...
3. `stdin.resolveDir`を基準にesbuildでバンドルする（内部IIFE、ES2018、browser platform）
   ...
   - dependency package内のrelative / absolute importは、symlink解決後もそのpackage root内だけに制限する
...
- npm packageは`resolveDir`（省略時は実行時のcurrent working directory）から解決する。
  ...
  `node_modules`を直接指すことはできず、dependency packageからのlocal importはsymlink解決後も
  自身のpackage rootを越えられない

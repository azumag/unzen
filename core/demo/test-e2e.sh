...
echo "🧪 unzen core E2E Integration Tests"
echo "===================================="
echo ""
...
if [[ $INDEX_HTML == *"unzen core E2E Demo"* ]]; then
...
# Test 10b: UI architecture — no inline onclick handlers, stylesheet separated
echo "🎨 Test 10b: demo page has no inline onclick / inline style block"
if [[ $INDEX_HTML == *"onclick="* ]]; then
	echo "❌ Demo page still uses inline onclick handlers"
	exit 1
fi
if [[ $INDEX_HTML == *"<style>"* ]]; then
	echo "❌ Demo page still has an inline <style> block"
	exit 1
fi
if [[ $INDEX_HTML == *"demo.css"* ]]; then
	echo "✅ Demo page links the separated stylesheet (demo.css)"
else
	echo "❌ Demo page does not link demo.css"
	exit 1
fi
echo ""

# Test 10c: New static modules are served
echo "📦 Test 10c: GET pure UI modules (demo-state.js / demo-stats.js / demo.css)"
for MODULE in demo.css demo-state.js demo-stats.js demo-validate.js demo-diagnostics.js demo-i18n.js; do
	STATUS=$(curl -s -o /dev/null -w "%{http_code}" ${BASE_URL}/${MODULE})
	if [[ $STATUS == "200" ]]; then
		echo "✅ ${MODULE} served (200)"
	else
		echo "❌ ${MODULE} returned ${STATUS}"
		exit 1
	fi
done
echo ""

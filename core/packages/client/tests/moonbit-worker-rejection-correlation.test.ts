...
function createState(): MoonbitWorkerState {
  return { compiledModules: new Map() };
}

describe('MoonBit rejected-message correlation boundary', () => {
    ...
    const request = {
      get protocolVersion() {
        return limitedGetter(
          reads,
          'protocolVersion',
          MOONBIT_WORKER_PROTOCOL_VERSION + 1,
          1,
        )();
      },
      get generationId() {

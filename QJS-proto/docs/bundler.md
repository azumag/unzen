...
モジュールがホワイトリストで許可されているか確認。Node.js 組み込みモジュールは常に `false`。

### `isNodeBuiltin(name): boolean`

Node.js 組み込みモジュールかどうかを判定。`node:` プレフィックスも対応。

### `checkForbiddenApis(code): string[]`

コード内の禁止API使用を検出。違反がなければ空配列を返す。
...
1. ソースコードから import 文を抽出（静的プリチェック）
2. 各 import をホワイトリスト + Node.js 組み込みブロックリストで検証
3. esbuild でバンドル（IIFE形式、ES2018ターゲット、ブラウザプラットフォーム）
   - `onResolve` プラグインで全モジュール解決を検証
4. バンドル出力に禁止APIがないかスキャン
5. run() 関数を抽出可能な形に変換
...
- ランタイムのサンドボックス（QuickJS）がこれらの API を提供しないことが最終的な安全保証
